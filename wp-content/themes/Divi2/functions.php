...
require_once( __DIR__ . '/inc/widget-rubric-posts-without-image.php');
require_once( __DIR__ . '/inc/form-research-widget.php');
...
function i_register(){
    ...
    register_widget( 'I_Widget_Rubric_Post_Without_Image' );
    register_widget( 'I_Widget_Order_Form' );
    ...
    foreach ($childrens as $children) {
        ...
        if (is_category($cat_name)) :?>
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr valign="top">
                    <td width="302">
                        <?php if (is_active_sidebar('et_pb_widget_area_10')) {
                            dynamic_sidebar('et_pb_widget_area_10');
                        }?>
                    </td>
...
function i_modal_form_handler(){
    ...
    $name_service = $_POST['id_service'] ? wp_strip_all_tags($_POST['id_service']) : 'nothing';
    if ($email) {
        wp_mail(get_option('admin_email'), 'Research order',
            $name . ' (' . $phone . ') ' . 'из организации ' . $org . ' заказал ' . $name_service . ' ,email:' . $email );
    }
